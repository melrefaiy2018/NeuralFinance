...
class SentimentAnalyzer:
    ...
    def __init__(self, ticker_symbol: str = "NVDA"):
        ...
        Args:
            ticker_symbol (str, optional): The stock ticker symbol. Defaults to 'NVDA'.
        """
        ...
        try:
            ...
            try:
                ...
                self.alpha_vantage_key = (
                    ALPHA_VANTAGE_API_KEY if ALPHA_VANTAGE_API_KEY != "YOUR_API_KEY_HERE" else None
                )
            except:
                self.alpha_vantage_key = None
    ...
    def fetch_news_sentiment(self, start_date, end_date) -> Optional[pd.DataFrame]:
        ...
        Args:
            start_date: The start date for fetching sentiment data.
            end_date: The end date for fetching sentiment data.

        Returns:
            Optional[pd.DataFrame]: The fetched sentiment data, or None if no data is available.
        """
        ...
        for method_name, method_func in methods:
            try:
                ...
                if result is not None and len(result) > 0:
                    ...
                    if (
                        "Synthetic" in method_name
                        or "Price-Technical Analysis" in method_name
                        or "Market Trend Sentiment" in method_name
                    ):
    ...
    def _can_use_alpha_vantage(self) -> bool:
        ...
        Returns:
            bool: True if the API can be used, False otherwise.
        """
    ...
    def _fetch_alpha_vantage_safe(self, start_date, end_date) -> Optional[pd.DataFrame]:
        ...
        Returns:
            Optional[pd.DataFrame]: The fetched sentiment data, or None if fetching fails.
        """
            ...
            if not news_data:
                ...
                for var in variations:
                    try:
                        ...
                        if news_data and len(news_data) > 0:
                            print(
                                f"Found {len(news_data)} news articles using ticker variation {var}"
                            )
                            break
                    except:
                        continue
    ...
    def _generate_price_technical_sentiment(self, start_date, end_date) -> pd.DataFrame:
        ...
        Args:
            start_date: The start date for generating sentiment data.
            end_date: The end date for generating sentiment data.

        Returns:
            pd.DataFrame: The generated sentiment data.
        """
        try:
            ...
            for i, date in enumerate(date_range):
                ...
                if len(current_data) >= 20:  # Need sufficient data for analysis
                    ...
                    sentiment_scores = self._calculate_sentiment_from_technicals(
                        current_data, latest
                    )
    ...
    def _generate_market_trend_sentiment(self, start_date, end_date) -> pd.DataFrame:
        ...
        try:
            ...
            try:
                stock_data = stock_ticker.history(
                    start=extended_start, end=end_date + pd.Timedelta(days=1)
                )
                market_data = {}

                for name, ticker in indices.items():
                    try:
                        market_data[name] = ticker.history(
                            start=extended_start, end=end_date + pd.Timedelta(days=1)
                        )
                    except:
                        continue
                ...
                for i, date in enumerate(date_range):
                    ...
                    sentiment_df.loc[i, "sentiment_positive"] = max(
                        0.1, min(0.8, 0.5 + trend_factor)
                    )
                    sentiment_df.loc[i, "sentiment_negative"] = max(
                        0.1, min(0.8, 0.3 - trend_factor)
                    )
    ...
    def _calculate_market_relative_sentiment(self, stock_data, market_data, date):
        ...
        try:
            ...
            stock_return = (
                recent_stock["Close"].iloc[-1] - recent_stock["Close"].iloc[0]
            ) / recent_stock["Close"].iloc[0]
            ...
            for name, data in market_data.items():
                try:
                    ...
                    if len(market_subset) >= 10:
                        ...
                        market_return = (
                            recent_market["Close"].iloc[-1] - recent_market["Close"].iloc[0]
                        ) / recent_market["Close"].iloc[0]
    ...
    def _process_yahoo_news_enhanced(self, news_data, start_date, end_date) -> pd.DataFrame:
        ...
        if article_count > 0:
            ...
            for i in range(len(sentiment_df)):
                ...
                sentiment_df.loc[i, "sentiment_negative"] = max(
                    0.05, min(0.95, neg_weight - noise / 2)
                )
                sentiment_df.loc[i, "sentiment_neutral"] = max(
                    0.05, min(0.95, neu_weight + noise / 3)
                )
