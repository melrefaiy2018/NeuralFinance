...
def fixed_evaluate(model, y_true_scaled, y_pred_scaled):
    ...
    if np.isnan(y_pred_scaled).any() or np.isinf(y_pred_scaled).any():
        ...
        y_pred_scaled = np.nan_to_num(y_pred_scaled, nan=median_val, posinf=median_val, neginf=median_val)
    ...
    if np.isnan(y_true_scaled).any() or np.isinf(y_true_scaled).any():
        ...
        y_true_scaled = np.nan_to_num(y_true_scaled, nan=median_val, posinf=median_val, neginf=median_val)
    ...
    try:
        ...
        if scale_diff > np.median(y_true_clean):
            ...
            scale_factor = np.median(y_true_clean) / np.median(y_pred_clean) if np.median(y_pred_clean) != 0 else 1
