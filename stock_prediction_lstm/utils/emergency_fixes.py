...
class ModelEvaluationFixer:
    ...
    def diagnose_evaluation_issues(y_true_scaled, y_pred_scaled, price_scaler):
        ...
        Args:
            ...
            price_scaler: The price scaler used for the model.

        Returns:
            bool: True if the diagnostics were successful, False otherwise.
        """
    ...
    def emergency_evaluate(y_true_scaled, y_pred_scaled, price_scaler, current_stock_price=None):
        ...
        Args:
            ...
            y_pred_scaled: The scaled predicted values.
            price_scaler: The price scaler used for the model.
            current_stock_price (float, optional): The current stock price. Defaults to None.
        ...
        try:
            ...
            try:
                ...
                print(
                    f"After inverse transform - True range: [{np.min(y_true_orig):.2f}, {np.max(y_true_orig):.2f}]"
                )
                print(
                    f"After inverse transform - Pred range: [{np.min(y_pred_orig):.2f}, {np.max(y_pred_orig):.2f}]"
                )
                ...
                if current_stock_price is not None:
                    ...
                    scale_factor = (
                        current_stock_price / np.mean(y_true_clean)
                        if np.mean(y_true_clean) != 0
                        else current_stock_price
                    )
            ...
            results = {"rmse": float(rmse), "mae": float(mae), "r2": float(r2), "mape": float(mape)}
...
def clean_predictions(y_true, y_pred):
    ...
    Returns:
        A tuple containing the cleaned true and predicted values.
    """
...
def try_scaling_corrections(y_true, y_pred):
    ...
    Returns:
        dict: A dictionary of evaluation metrics, or None if the corrections fail.
    """
    try:
        # Try normalizing both predictions to same scale
        if np.std(y_true) > 0 and np.std(y_pred) > 0:
            y_pred_normalized = (y_pred - np.mean(y_pred)) / np.std(y_pred) * np.std(
                y_true
            ) + np.mean(y_true)
...
def is_reasonable(metrics):
    ...
    Returns:
        bool: True if the metrics are reasonable, False otherwise.
    """
    return (
        -10 <= metrics["r2"] <= 1
        and 0 <= metrics["rmse"] <= 1000
        and 0 <= metrics["mae"] <= 1000
        and 0 <= metrics["mape"] <= 1000
    )
