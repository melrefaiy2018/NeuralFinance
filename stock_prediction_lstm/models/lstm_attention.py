...
class StockSentimentModel:
    ...
    def __init__(self, look_back: int = 20, forecast_horizon: int = 1):
        """
        Initializes the StockSentimentModel.

        Args:
            look_back (int, optional): The number of previous time steps to use as input. Defaults to 20.
            forecast_horizon (int, optional): The number of future time steps to predict. Defaults to 1.
        """
    ...
    def prepare_data(
        self, df: np.ndarray, target_col: str = "close"
    ) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        Prepares the data for training the model.

        Args:
            df (np.ndarray): The input DataFrame.
            target_col (str, optional): The name of the target column. Defaults to 'close'.

        Returns:
            Tuple[np.ndarray, np.ndarray, np.ndarray]: A tuple containing the market data, sentiment data, and target data.
        """
    ...
    def _create_sequences(
        self, market_data: np.ndarray, sentiment_data: np.ndarray, target_data: np.ndarray
    ) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        Creates sequences of data for training the model.

        Args:
            market_data (np.ndarray): The market data.
            sentiment_data (np.ndarray): The sentiment data.
            target_data (np.ndarray): The target data.

        Returns:
            Tuple[np.ndarray, np.ndarray, np.ndarray]: A tuple containing the market data, sentiment data, and target data sequences.
        """
    ...
    def build_model(self, market_input_dim: int, sentiment_input_dim: int) -> Model:
        ...
        Args:
            market_input_dim (int): The input dimension of the market data.
            sentiment_input_dim (int): The input dimension of the sentiment data.
        ...
        market_attention = MultiHeadAttention(num_heads=4, key_dim=16)(
            market_dropout2, market_dropout2
        )
        ...
        if hasattr(self, "has_sentiment") and self.has_sentiment:
            ...
            sentiment_attention = MultiHeadAttention(num_heads=2, key_dim=8)(
                sentiment_dropout2, sentiment_dropout2
            )
    ...
    def fit(
        ...
        Args:
            X_market (np.ndarray): The market training data.
            X_sentiment (np.ndarray): The sentiment training data.
            ...
            callbacks (list, optional): A list of callbacks. Defaults to None.
    ...
    def predict(self, X_market: np.ndarray, X_sentiment: np.ndarray) -> np.ndarray:
        ...
        Args:
            X_market (np.ndarray): The market data to predict on.
            X_sentiment (np.ndarray): The sentiment data to predict on.
    ...
    def predict_next_days(
        self, latest_market_data: np.ndarray, latest_sentiment_data: np.ndarray, days: int = 5
    ) -> List[float]:
        """
        Predicts the next n days of stock prices.

        Args:
            latest_market_data (np.ndarray): The latest market data.
            latest_sentiment_data (np.ndarray): The latest sentiment data.
            days (int, optional): The number of days to predict. Defaults to 5.

        Returns:
            List[float]: A list of predicted prices.
        """
        predictions = []
        ...
        for i in range(days):
            try:
                ...
                next_day_transformed = self.price_scaler.inverse_transform(
                    next_day_scaled.reshape(-1, 1)
                )[0][0]
                ...
                if i == 0:
                    if next_day_price < current_price * 0.5 or next_day_price > current_price * 2.0:
                        scale_factor = (
                            current_price / next_day_price if next_day_price != 0 else 1.0
                        )
