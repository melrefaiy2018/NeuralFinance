...
def visualize_stock_data(df, ticker_symbol, output_dir=None):
    """
    Visualizes stock data, including price, volume, and technical indicators.
    ...
    if "sentiment_positive" in df.columns and "sentiment_negative" in df.columns:
        ...
        ax5.plot(
            df["date"], df["sentiment_positive"], label="Positive", color="green", linewidth=1.5
        )
        ax5.plot(df["date"], df["sentiment_negative"], label="Negative", color="red", linewidth=1.5)
        if "sentiment_neutral" in df.columns:
            ax5.plot(
                df["date"], df["sentiment_neutral"], label="Neutral", color="gray", linewidth=1.5
            )
...
def visualize_prediction_comparison(
    model, X_market_test, X_sentiment_test, y_test, ticker_symbol, output_dir=None
):
    """
    Visualizes the comparison between predicted and actual stock prices.

    Args:
        model: The trained model.
        X_market_test: The market test data.
        X_sentiment_test: The sentiment test data.
    ...
    if hasattr(model, "transformed_price_data") and hasattr(model, "original_price_data"):
        if (model.original_price_data > 0).all() and np.mean(
            model.transformed_price_data
        ) < np.mean(model.original_price_data):
    ...
    if scale_ratio > 10 or scale_ratio < 0.1:
        print(
            f"WARNING: Scale discrepancy detected! true_mean={true_mean:.4f}, pred_mean={pred_mean:.4f}, ratio={scale_ratio:.4f}"
        )
    ...
    plt.title(
        f"{ticker_symbol} Stock Price Prediction\n"
...
def visualize_future_predictions(
    future_prices, future_dates, df, ticker_symbol, window=30, output_dir=None
):
    """
    Visualizes future stock price predictions.

    Args:
        future_prices (list): The list of predicted future prices.
        future_dates (list): The list of future dates.
        df (pd.DataFrame): The DataFrame containing the historical stock data.
        ticker_symbol (str): The stock ticker symbol.
        window (int, optional): The number of historical days to display. Defaults to 30.
        output_dir (str, optional): The directory to save the plot. Defaults to None.
    ...
    if percent_diff > 10:
        ...
        print(
            f"Last historical price: ${last_hist_price:.2f}, First prediction: ${first_pred_price:.2f}"
        )
        ...
        print(
            f"Adjusted prediction range: ${np.min(future_prices_adjusted):.2f} - ${np.max(future_prices_adjusted):.2f}"
        )
    ...
    pred_return = (
        (future_prices[-1] - future_prices[0]) / future_prices[0] * 100
        if future_prices[0] != 0
        else 0
    )
    ...
    for i, (date, price, orig_price) in enumerate(
        zip(future_dates, future_prices_adjusted, future_prices)
    ):
        if scaling_applied:
            plt.annotate(
                f"${price:.2f}\n(${orig_price:.2f})",
...
def visualize_feature_importance(df, target_col="close", output_dir=None):
    """
    Visualizes the feature importance by calculating the correlation with the target column.
...
def visualize_sentiment_impact(df, window=14, output_dir=None):
    """
    Visualizes the impact of sentiment on stock price.

    Args:
        df (pd.DataFrame): The DataFrame containing the stock and sentiment data.
        window (int, optional): The rolling window for correlation. Defaults to 14.
        output_dir (str, optional): The directory to save the plot. Defaults to None.
